...
void FingerPS_AuraLedConfig (void){
	...
	UART_sendByte(LIGHT_GRADUALLY_ON); 
	UART_sendByte(MEDIUM_SPEED);
	UART_sendByte(COLOR_INDEX_RED); 
	UART_sendByte(LIGHT_COUNT); 
	UART_sendByte((PCK_ID_COMMAND_PACK+LENGTH_7BYTE+AURA_LED_CONFIG+LIGHT_GRADUALLY_ON+MEDIUM_SPEED+COLOR_INDEX_RED+LIGHT_COUNT)>>ONE_BYTE_SHIFT);
	UART_sendByte((PCK_ID_COMMAND_PACK+LENGTH_7BYTE+AURA_LED_CONFIG+LIGHT_GRADUALLY_ON+MEDIUM_SPEED+COLOR_INDEX_RED+LIGHT_COUNT)>>NO_BYTE_SHIFT);	
}
